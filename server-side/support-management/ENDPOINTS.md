...
| EP-5 | `PATCH` | `/api/user-requests/{id}/solve` | Complete an assigned support request as the authenticated actor. | Request ID; actor comes from authentication | `UserRequestService` | `solveRequest` |
| EP-6 | `PATCH` | `/api/user-requests/{id}/reject` | Reject a pending or assigned request as authenticated support staff. | Request ID; actor comes from authentication | `UserRequestService` | `rejectRequest` |
